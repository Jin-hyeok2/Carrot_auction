...
public class MailServiceImpl implements MailService {
    ...
    private MimeMessage createMessage(Long userId) throws Exception {
        ...
        content.append(String.format("<h1> 안녕하세요, %s님! </h1>", user.getNickname()));
        content.append("<h1> 가입 완료를 위해 아래 링크를 눌러주세요</h1>");
        content.append(String.format("<a href=%s?id=%d&authCode=%s>이메일 인증</a>", emailAuthURL, userId, user.getAuthCode()));
        content.append("</div>");
